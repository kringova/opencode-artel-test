...
Назначь `model_tier` — грейд модели, который по умолчанию возьмёт задачу. Выводится из **природы задачи (механика vs рассуждение), не из важности по RICE**. Три грейда вендоронейтральны (маппинг на конкретные модели — в адаптере инструмента); методология — раздел «Тиринг моделей» в `docs/methodology.md`.

- `senior` — если **любое**: `confidence ≤ 60` (мутная/гипотеза) ИЛИ включена роль `analyst` ИЛИ задача меняет архитектуру/контракт. Это рассуждательная работа.
- `junior` — если **всё**: `confidence ≥ 80` И `est_days ≤ 0.5` И не меняет архитектуру И нет роли `analyst` И DoD — чёткий наблюдаемый результат. Это механика/шаблон.
- `middle` — **дефолт**, всё остальное.

Грейд — плановый: при исполнении возможна эскалация на ступень вверх (`junior → middle → senior`). Пороги `est_days`/confidence стартовые, самокалибруются по доле эскалаций.

Пример: `confidence 65, est_days 0.3` → `model_tier: middle` (confidence ниже 80 → не junior, но выше 60 → не senior). Задача с `analyst` или сменой архитектуры → `senior`.
