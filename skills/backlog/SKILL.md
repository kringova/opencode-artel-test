...
- `junior` — если **всё**: `confidence ≥ 80` И `sp ≤ 2` И не меняет архитектуру И нет роли `analyst` И DoD — чёткий наблюдаемый результат. Это механика/шаблон.
- `middle` — **дефолт**, всё остальное.

Грейд — плановый: при исполнении возможна эскалация на ступень вверх (`junior → middle → senior`). Пороги `sp`/confidence стартовые, самокалибруются по доле эскалаций.

Пример: `confidence 65, sp 2` → `model_tier: middle` (confidence ниже 80 → не junior, но выше 60 → не senior). Задача с `analyst` или сменой архитектуры → `senior`.
